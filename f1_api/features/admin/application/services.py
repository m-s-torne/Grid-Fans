    ...
    async def execute(self) -> dict:
        ...
        try:
            ...
            with Session(self.engine) as session:
                ...
                drivers: list[Drivers] = get_driver_data(session, driver_repo, season_context)
                
                session.add_all(drivers)
                session.commit()

                all_driver_team_links = get_all_driver_team_links(
                ...
                missing_links = await reconcile_driver_team_links(
                    session, year, driver_repo, team_repo, link_repo, season_context
                )
                if missing_links:
